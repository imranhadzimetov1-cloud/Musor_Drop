...
function loadState() {
    ...
    try {
        ...
        if (savedData) {
            ...
            console.log('✅ Загружено! Баланс:', state.balance);
        } else {
...
function saveState() {
    try {
        ...
        console.log('💾 Сохранено! Баланс:', state.balance);
    } catch (e) {
...
window.sellSkin = function(skinId) {
    ...
    saveState();
    showToast(`Продано: ${inventoryItem.name} за ${currentMarketPrice} R!`);
};
...
// ==========================================
// БЕЗОПАСНАЯ РАБОТА С GITHUB API (без config.js)
// ==========================================
...
// ==========================================
// 1. АДМИН ВХОДИТ (вводит токен 1 раз)
// ==========================================
function adminLogin() {
    ...
    if (token.startsWith('ghp_')) {
        ...
        alert('✅ Токен сохранен в браузере! Теперь вы можете синхронизировать цены.');
        showToast('✅ Вы вошли как администратор!');
        updateAdminStatus();
    } else {
        alert('❌ Неверный формат токена! Токен должен начинаться с "ghp_"');
    }
}

// ==========================================
// 2. ПРОВЕРКА СТАТУСА АДМИНА
// ==========================================
...
function updateAdminStatus() {
    ...
    if (statusEl) {
        if (isAdminLoggedIn()) {
            statusEl.innerHTML = '✅ Админ: Вход выполнен';
            statusEl.style.color = '#22c55e';
        } else {
            statusEl.innerHTML = '❌ Админ: Не авторизован';
...
// ==========================================
// 3. СОХРАНЕНИЕ ЦЕН НА GITHUB
// ==========================================
async function savePricesToGitHub() {
    const token = localStorage.getItem('github_admin_token');
    
    if (!token) {
        showToast('⚠️ Требуется авторизация администратора!');
        if (confirm('Вы не авторизованы. Войти как администратор?')) {
            adminLogin();
        }
        return;
    }
    ...
    try {
        ...
        if (putResponse.ok) {
            ...
            console.log('✅ Цены сохранены на GitHub!');
        } else {
...
// ==========================================
// 4. ЗАГРУЗКА ЦЕН С GITHUB (для всех пользователей)
// ==========================================
    ...
    try {
        ...
        if (!response.ok) {
            if (response.status === 404) {
                console.log('📭 Файл с ценами пока не создан');
                return false;
            }
        ...
        if (marketData.prices) {
            ...
            console.log('✅ Цены загружены с сервера! Обновлено:', Object.keys(marketData.prices).length, 'скинов');
            renderShop();
            renderInventory();
            saveState();
            return true;
...
// ==========================================
// 5. АДМИН-КНОПКИ ДЛЯ СИНХРОНИЗАЦИИ
// ==========================================
...
// ==========================================
// Добавьте в функцию loadState() или в DOMContentLoaded:
